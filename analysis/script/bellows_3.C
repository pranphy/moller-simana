...
std::string opfile="./asset/files/root/bellows3-cfg07-plots-ab.root";
...
auto corr_hist(int det1, int det2, hit_cut basecut=utl::cut::epm, std::string id="E(all)"){
    ...
    return hst::hist(name,msc::rr_correlate(det1,det2,basecut),title,{300,40,350,300,10,335});
}


auto tracks_on(int det){
    float rmin = 240, rmax = 292, Emin = 240, Emax = 650;
    auto title = std::string(Form("Tracks on %s from [r(%.1f,%.1f), E(%.1f,%.1f)@Bellows3]; x[mm];y[mm]",detname[det].c_str(),rmin,rmax,Emin,Emax));
    auto name = std::string(Form("%d-tr-%.0f-%.0f-%.0f-%.0f",det,rmin,rmax,Emin,Emax));
    return hst::hist(name,msc::track_selector(msc::all({msc::E_cut(Emin,Emax),msc::r_cut(rmin,rmax),bellows_3_us_epm})),utl::cut::detid(det),{att::x,att::y},title,{300,-300,300,300,-300,300});
}

// Only possible 1d and 2d histograms.

std::vector<hst::hist> histograms{
    hst::hist("r-bellows3-us",msc::all({bellows_3_us_epm,msc::E_cut(100,400)}),{att::r},"Bellows3 US (100-400)MeV; r[mm]; Count",{100,0,355}), 
    hst::hist("r-bellows3-ds",msc::all({bellows_3_ds_epm,msc::E_cut(100,400)}),{att::r},"Bellows3 DS (100-400)MeV; r[mm]; Count",{100,0,355}), 
    ...
    corr_hist(39,38), corr_hist(39,44), corr_hist(39,45), corr_hist(39,46), corr_hist(39,47), 
    corr_hist(39,483), corr_hist(39,484), corr_hist(483,44),
    ...
    tracks_on(38), tracks_on(39), tracks_on(44), tracks_on(45), tracks_on(47), tracks_on(484),
};


void make_plots(ROOT::RDataFrame& df){
