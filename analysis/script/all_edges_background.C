...
const static Boxes components{ // for each components {vz0, vz1, r0, r1}
    ...
    {"Collimator 2",            {735.0, 849,30,120}},
    {"Two bounce shield",       {924, 3500, 20,65}}, // For real.
    {"Collimator 4",            {3200, 3350, 182,195}},
    ...
    {"Collimator 5",            {7554, 7772, 40, 300 }},
    {"Collimator 6A",           {9550.0,9720.0,52,80.0}},
    {"Lintel",                  {9642.49,9748.49, 475.0, 824.0}},
    {"Collimator 6B",           {10925.0,11100.0,79.0,120.0}},
    {"Collar 1",                {11600.0,12000.00,540,760}},
    {"Vaccum Enclosure DS End", {12000, 12180, 700,1300}},
    {"Bellows 4 Region",        {12200, 13600, 600,1300}},
    {"Vaccum Window",           {18400, 18750, 490, 550}},
    {"Unnamed ",                {18550.0,18750.0,700.0,900.0}},
    {"Bellows 5",               {18900.0,19400.0,400.0,600.0}},
    {"Det. Beam Pipe",          {19300.0,27000.0,500.0,640.0}},
    {"Collar 2",                {18932.0,19320.0,1009.0,1040.0}},
...
void init_histmap(const Boxes& boxes, Hist2Ds& hists){
    ...
    for(auto box : boxes){
        ...
        auto get_th2d = [&xy, &i](std::string name){ return new TH2D(Form("vzvr%02d",i++),(name+ " ; vz[mm]; vr[mm]").c_str(),100,xy[0], xy[1], 100,xy[2], xy[3]); };
        auto get_xy  =  [&xy, &i](std::string name){ return new TH2D(Form("yx%02d",i++),(name+ " ; vx[mm]; vy[mm]").c_str(),100,-xy[3], xy[3],100,-xy[3],xy[3]); };
...
void save_hists(Hist2Ds& hists, const std::string& id, const std::string &opfile){
    int i = 0;
    std::cout<<"Name, Count, Moller Fraction"<<std::endl;
    for(auto hist: hists) {
        int count = hist.second->Integral();
        std::string name = hist.first;
        std::cout<<hist.first<<", "<<hist.second->GetXaxis()->GetXmin()<<", "<<count<<", "<<utl::moller_fraction(count,7.609e8)*100<<" "<<std::endl;
        utl::save(hist.second,Form("vz-vr-for-%02d",++i),id+"/plot",opfile);
    }
}
...
void make_all_plots(std::string& filename, const std::string& id, const std::string& opfile){
    ROOT::EnableImplicitMT();
    ROOT::RDataFrame d("T",example_file);
    Hist2Ds histmap;

    auto the = new TH1D("vzhiste","All background $e^{\\pm}$; vz[mm]; count",200,-3000, 28000);
    auto thp = new TH1D("vzhistp","All background $\\gamma$; vz[mm]; count",200,-3000, 28000);
    auto exc = new TH2D("vzvrpl","All background; vz[mm]; vr[mm]", 200, -3500,25000.0,200, 0.0,1500.0);
...
void bly_df(){
    static const std::string opfile = "asset/files/bill_info_43_df_test.root";
    std::string filename = example_file;
    static const std::string id = "col2-cfg08";
    make_all_plots(filename, id, opfile);
